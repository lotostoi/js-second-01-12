<template>
  <div class="menuTop__contCart">
    <div
      v-on:click="
        $parent.openCart = !$parent.openCart;
        $parent.openMenu = false;
      "
      class="menuTop__CartSvg"
    ...
    <div
      class="contCartProducts"
      :class="{ 'contCartProducts-active': $parent.openCart }"
      id="contCartProducts"
    >
...
export default {
  ...
  inject: ["getData", "allFetch"],
  props: ["openCart", "openMenu"],
  data() {
    return {
      cartURL: "/cart.json",
  ...
  methods: {
    cleanCart() {
      this.allFetch(this.delFromCart, "DELETE", '{"allQuntity":"0"}').then(
        data => {
          if (data.result == 1) {
            this.cartItems = [];
          } else {
        ...
        this.allFetch(
          ...
          if (data.result == 1) {
            this.cartItems.splice(
              this.cartItems.findIndex(index => index.id == find.id),
              1
            );
          } else {
            console.log("Сервер не отвечает - 404!");
  ...
  mounted() {
    this.getData(this.cartURL).then(data => {
